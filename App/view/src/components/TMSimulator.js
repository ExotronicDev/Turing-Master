...
class TuringControlError extends Error {
	...
	format(str) {
		for (let k in this.info) str = str.replace("{" + k + "}", this.info[k]);
		return str;
	}
...
class TuringControl {
	...
	defaults() {
		...
		var ofst = this.initialOffset,
			str = this.initialTape;
	...
	needs(requirements) {
		if (typeof requirements.programmed !== "undefined") {
			if (
				typeof this.programming === "undefined" &&
	...
	compute() {
		...
		if (!Worker) throw new TuringControlError(12);
		...
		this.running = true;
		this.skipping = true;
		this.skipworker.postMessage(
			new TuringControlMessage("start", {
	...
	init(program, initial, offset) {
		var COMMAND_PATTERN =
			/^\s*([0-9A-Za-z]{1,3}),([^, ]) +([0-9A-Za-z]{1,3}),([^, ])(?:,([>_<]))?\s*(?:#.*)?$/;

		if (this.running || this.skipping) throw new TuringControlError(1);

		offset = typeof offset === "undefined" ? 0 : Number(offset);
		if (!isFinite(offset)) throw new TuringControlError(14);
		...
		for (var l = 0; l < lines.length; l++) {
			...
			if (!match) throw new TuringControlError(2, { line: l + 1 });
			...
			if (!programming[state]) programming[state] = {};
			...
			if (!calledByLine[targetState]) calledByLine[targetState] = l + 1;

			empty = false;
		}

		if (empty) throw new TuringControlError(5);

		// check for calls to undefined states
		var definedStates = Object.keys(programming);
		for (var state of calledStates) {
			if (
				definedStates.indexOf(state) === -1 &&
				state !== this.HALT_STATE
			)
				throw new TuringControlError(6, {
					state: state,
					line: calledByLine[state],
				});
		}
	...
	transition() {
		...
		if (char === " ") char = this.BLANK_SYMBOL;
		...
		if (!targetState) {
			...
			var event = new CustomEvent("runtimeerror", {
				detail: new TuringControlError(13, {
					state: this.state,
					symbol: char,
				}),
			});
		...
		this.setTape(
			this.position,
			newChar === this.BLANK_SYMBOL ? " " : newChar
		);
	...
	export(env) {
		...
		for (let k of this.STATE_LIST) ex[k] = this[k];
		if (env) for (let k of this.ENV_LIST) ex[k] = this[k];
		return ex;
	}
	import(im, env) {
		for (let k of this.STATE_LIST) this[k] = im[k];
		if (env) for (let k of this.ENV_LIST) this[k] = im[k];
	}
	...
	removeEventListener(type, callback) {
		if (type in this.listeners) {
			this.listeners[type].forEach(function (item, i, arr) {
				if (item === callback) arr.splice(i, 1);
			});
		}
	}
	dispatchEvent(event) {
